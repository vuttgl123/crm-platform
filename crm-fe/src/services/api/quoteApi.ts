...
export const QUOTE_STATUS_CONFIG: Record<QuoteStatus, { label: string; className: string }> = {
  ...
  PENDING_APPROVAL: { label: 'Chờ duyệt', className: 'bg-amber-50 text-amber-700 border-amber-200 font-semibold' },
  APPROVED: { label: 'Đã duyệt', className: 'bg-blue-50 text-blue-700 border-blue-200 font-bold' },
  SENT: { label: 'Đã gửi khách', className: 'bg-purple-50 text-purple-700 border-purple-200 font-bold' },
  ACCEPTED: { label: 'Đã chấp thuận', className: 'bg-emerald-50 text-emerald-700 border-emerald-200 font-bold' },
  REJECTED: { label: 'Từ chối', className: 'bg-rose-50 text-rose-700 border-rose-200 font-semibold' },
  EXPIRED: { label: 'Hết hạn', className: 'bg-slate-200 text-slate-700 border-slate-300' },
  CANCELLED: { label: 'Đã hủy', className: 'bg-rose-100 text-rose-800 border-rose-300 font-semibold' },
};
