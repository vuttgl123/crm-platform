...
import { ContactSummaryResponse, ContactResponse } from '@/services/api/contactApi';
import { AlertTriangle, Loader2, Trash2 } from 'lucide-react';

interface ContactDeleteDialogProps {
  isOpen: boolean;
  contact: ContactSummaryResponse | ContactResponse | null;
  isDeleting: boolean;
  onClose: () => void;
  onConfirmDelete: (contact: ContactSummaryResponse | ContactResponse) => void;
}

export const ContactDeleteDialog: React.FC<ContactDeleteDialogProps> = ({
  isOpen,
  contact,
  isDeleting,
  onClose,
      ...
      <AlertDialogContent className="max-w-md font-sans">
        <AlertDialogHeader className="space-y-2">
          ...
          <AlertDialogDescription className="text-xs text-slate-600 space-y-2">
            <span>
              ...
              <strong className="text-slate-900 font-semibold">{contact.displayName}</strong>{' '}
              (<span className="font-mono text-slate-700">{contact.contactNumber}</span>)?
            </span>
          </AlertDialogDescription>
        </AlertDialogHeader>
        ...
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-[4px] space-y-1 text-xs text-amber-800">
          ...
          <p className="text-[11px] leading-relaxed">
            This contact and its associations will be permanently removed from the system. This action cannot be undone.
          </p>
        </div>
